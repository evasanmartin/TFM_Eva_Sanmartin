# -----------------------------------------------------------------------
# This script creates a beautiful heatmap from carotenoid metabolism gene 
# abundances and adds a barplot with completeness data. Uses carotenoid
...
df <- read.table("Funciones/carotene_table.tsv", sep = "\t", check.names = F)

df.long <- as.data.frame(df) %>% 
           rownames_to_column(var = "Bin") %>% 
           gather(KO, Copy_number, 2:(length(colnames(df))+1))

df2 <- read.table("Funciones/carotene_comp_table.tsv", sep = "\t", check.names = F)
...
heatmap <- ggplot(df.long, aes(x = KO, y = Bin, fill = Copy_number)) +
    ...
    scale_fill_gradientn(colours = paletteer_d("ggsci::orange_material"), 
                         values = scales::rescale(c(0,0.01,0.3,0.6,1,1.5,2,3,4,5))) +
    xlab(label = "") +
    ylab (label = "") +
    labs(fill="Abundance\n(copy number)") +
    ...
    scale_x_discrete(limits = colnames(table)) + # Ordering genes
    theme(
...
ggsave("carotene_heatmap.png", width = 15, height = 25, units = "cm", bg = NULL)

# ------------------------
# Barplot

barplot <- ggplot(df.long2, aes(x = Completeness/7, y = Bin)) +
    ...
    scale_x_continuous(limits = c(0,101), breaks = seq(0,100,25)) +
    theme(
        text = element_text(family = "Poppins"),
        panel.background = element_blank(),
        ...
        axis.ticks.y = element_blank(), # Remove Y axis text
        plot.margin = margin(t = 20, r = 40)
    )

ggarrange(heatmap, barplot, ncol = 2, nrow = 1, widths = c(1,0.3), align = "h")

ggsave("carotene_heatmap_barplot.png", width = 20, height = 30, units = "cm", bg = "white")
ggsave("carotene_heatmap_barplot.svg", width = 20, height = 30, units = "cm")
