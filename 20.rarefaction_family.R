# -----------------------------------------------------------------------------
# Script for counting the reads that belong to each family in the sample from 
...
for (sample in c("20IB32", "23IB32")) {
    zz <- projs[[sample]]$contigs
    nono <- c(grep("[Nn]o ", zz$tax[,"family"]), # Search for "(no family in NCBI)" or "No CDS"
              grep("Unclassified", zz$tax[,"family"], fixed = T)) # Search for "Unclassified"
    yes <- setdiff(1:nrow(zz$table), nono) # Rows that aren't no family or unclassified (the rows that we want)
    family_v <- zz$tax[yes,"family"]
    sample.df <- data.frame(family_v, count = zz$abund[yes,1], stringsAsFactors = F) # Combine family and abundance (reads)
    sample.df <- rbind(sample.df, none = c("none", sum(zz$abund[nono,1]))) # Add NA count (no family or unclassified)
    rm(family_v)
    sample.df[,"count"] <- as.numeric(sample.df[,"count"])
    sample.sum <- sample.df %>%
                  group_by(family_v) %>% # Group by family
                  summarise(sum = sum(count)) # Sum of all reads in that family
    rm(sample.df)    
    sample.final[[sample]] <- as.data.frame(sample.sum[,2]) # Get counts by family
    # sample.final[[sample]] <- t(sample.final[[sample]]) # Transpose
    # colnames(sample.final[[sample]]) <- as.data.frame(sample.sum)[,1] # Families to column names
    rownames(sample.final[[sample]]) <- as.data.frame(sample.sum)[,1] # Families to row names
    rm(sample.sum)
    # write.table(sample.final[[sample]], file = paste("Diversity_table_family_raw_", sample, ".tsv", sep = ""), quote = F, col.name = F, sep = "\t")
}
...
alldiv <- c(`20IB32` = "Rarefaction/Diversity_table_family_raw_20IB32.tsv", 
            `23IB32` = "Rarefaction/Diversity_table_family_raw_23IB32.tsv")
...
# Save the table
# write.table(merged_20_23, "Rarefaction/Diversity_family_raw_all.tsv", sep = "\t", quote = F)
...
# merged_20_23 <- read.table("Rarefaction/Diversity_family_raw_all.tsv", sep = "\t", header = T, quote = "")
...
ggsave("Rarefaction_reads_family.png", width = 25, height = 20, units = "cm")
ggsave("Rarefaction_reads_family.svg", width = 25, height = 20, units = "cm")
